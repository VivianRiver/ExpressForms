...
namespace ExpressForms.Filters
{
    public class ExpressFormsFilterText : ExpressFormsFilter
    {
        ...
        public override void GenerateFilterIl(System.Reflection.Emit.ILGenerator generator, Dictionary<string, string> filterValues, PropertyInfo property)
        {
            ...
            // Do nothing for this property is the filterText is blank and the filterMode is "StartsWith" or "Contains"
            if (string.IsNullOrWhiteSpace(filterText) && new string[] { "StartsWith", "Contains" }.Contains(filterMode))
                return;
            ...
            switch (filterMode)
            {
                ...
                case "Contains":
                    ...
                    generator.Emit(OpCodes.Callvirt, filterMode == "StartsWith" ? StringStartsWith : StringContains);
